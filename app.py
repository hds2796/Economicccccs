...
def get_clean_error(e, model_name):
    ...
    if "503" in error_str: return "구글 서버 일시 과부하 (503 Service Unavailable)"
    if "404" in error_str: return f"API 모델명 인식 불가 (404 Not Found: {model_name})"
...
def build_prompt_recommend_step3(candidate_context, news_list, market_data_str):
    ...
    return (f"당신은 객관적이고 엄격한 수석 애널리스트입니다.\n\n"
            ...
            f"⚠️ [절대 준수 로직: 1차 퀀트 산출 -> 2차 정성적 조정]\n"
            f"목표가 산출 시, **반드시 1차적으로 퀀트(재무 수치, 보조지표 수식 등)를 활용하여 기계적인 가격을 도출한 후, 2차적으로 정성적 요인(뉴스, 수급, 매크로 등)을 반영해 그 목표가를 현실적으로 수정(가감)하는 과정**을 명시하십시오. 단기, 중기, 장기 모두 이 2단계 프로세스를 거쳐야 합니다.\n\n"
            f"[보고서 필수 양식]\n"
            f"### 🗑️ [탈락 종목 2개]\n"
            f"- [탈락 종목명 1, 2]: (제외 사유)\n\n"
            f"### 🏆 [최종 추천 종목 3개]\n"
            f"1. 🥇 추천종목: [종목명] (티커)\n"
            f"- 🎯 단기 목표가 (1~3개월): [최종 수정된 가격] (1차 퀀트: 수식 및 수치 -> 2차 정성: 뉴스/수급 요인을 통한 가감 조정)\n"
            f"- 🎯 중기 목표가 (3~6개월): [최종 수정된 가격] (1차 퀀트: 멀티플 연산 등 -> 2차 정성: 비즈니스/산업 모멘텀을 통한 가감 조정)\n"
            f"- 🎯 장기 목표가 (1년 이상): [최종 수정된 가격] (1차 퀀트: 내재가치 연산 -> 2차 정성: 매크로/구조적 변화를 통한 가감 조정)\n"
...
def validate_target_price(items):
    ...
    prompt = (
        "너는 독립적인 리스크 심사역이야. 입력된 종목별 목표가가 시장 현재가 및 실시간 기술적 수치 대비 타당한지 객관적으로 검증해라.\n"
        "괴리율이 비정상적이거나 RSI/MACD 수치와 방향성이 모순될 경우 반드시 'valid'를 false로 설정해라.\n\n"
        "⚠️ [필수 의무 사항: 1차 퀀트 산출 -> 2차 정성 조정]\n"
        "valid가 false일 경우, 'calculation_logic'에 재계산 로직을 적을 때 **반드시 [1차 퀀트(수치 기반 기계적 연산)] 후 [2차 정성(모멘텀/수급에 따른 가감)] 과정을 거쳐 도출된 최종 결과**라는 점을 단기/중기/장기별로 분명하게 작성해라.\n\n"
        ...
        '    "calculation_logic": "[단기] 1차 퀀트(볼린저 상단 19만) -> 2차 정성(단기 차익 매물 출회 가능성 반영하여 -5k 하향). [중기] 1차 퀀트(적정 PER 12배 연산 20만) -> 2차 정성(섹터 모멘텀 둔화 우려로 -5k 하향). [장기] 1차 퀀트(할인율 10% DCF 연산 22만) -> 2차 정성(장기 펀더멘털 유지 판단으로 조정 없음)."\n'
        '  }\n'
        '}\n\n'
        f"{context_block}"
    )
    ...
    try:
        ...
        for it in items:
            ...
            if v.get("valid") is False and not new_tp_s:
                ...
                calc_logic = "[단기] 1차 퀀트(+5% 밴드) -> 2차 정성(유지). [중기] 1차 퀀트(+10% 밴드) -> 2차 정성(유지). [장기] 1차 퀀트(+20% 밴드) -> 2차 정성(유지)."
...
def build_prompt_deep_dive(stock_name, ticker, news_list, is_owned, avg_price, quantity, current_price, market_data_str, tech_str, supply_str):
    ...
    return (f"[{stock_name} 진단]\n"
            ...
            f"   (※ 반드시 '1차 퀀트(수치) 산출 후 -> 2차 정성적 요인 조정' 프로세스를 거쳐서 단기/중기/장기를 서술할 것)\n"
            f"   - 단기(1~3개월): [가격] (1차 퀀트 연산 -> 2차 정성적 조정 내용)\n"
            f"   - 중기(3~6개월): [가격] (1차 퀀트 연산 -> 2차 정성적 조정 내용)\n"
            f"   - 장기(1년 이상): [가격] (1차 퀀트 연산 -> 2차 정성적 조정 내용)\n\n"
    ...
    if st.button("🚀 추천 종목 발굴", type="primary", use_container_width=True):
        ...
        try:
            ...
            def fetch_candidate_data(c_info):
                ...
                tech = raw_calculate_technical_indicators(t, realtime_price=cp)
            ...
            cand_lookup = {r["name"]: r for r in cand_results}
