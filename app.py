...
# =======================================================
# 2. 구글 드라이브 통합 데이터 로더 & 수동 백업 사이드바
# =======================================================
...
market_data_str = ", ".join([f"{k}: {v['current']}({v['diff_pct']}%)" for k, v in market_data.items()])

# 메인 지표 렌더링 (S&P 500 오류 방어 코드 포함)
...
for i, key in enumerate(target_indices):
    with cols[i]:
        if key in market_data:
            ...
            st.metric(label=key, value=f"{data['current']:,.2f}", delta=f"{data.get('diff', 0):,.2f} ({data.get('diff_pct', 0):.2f}%)")
        else:
            if key == "S&P 500":
...
with tab1:
    ...
    if st.button("🤖 실시간 뉴스 기반 종합 분석", type="primary", use_container_width=True):
        ...
        st.session_state.realtime_analysis = st.write_stream(call_gemini_stream_with_fallback(prompt))
    for news in news_list:
        with st.expander(f"🕒 {news['title']}"): st.markdown(f"[원문 읽기]({news['link']})\n\n{news['summary']}")
...
with tab5:
    ...
    for p in portfolios:
        ...
        with col_info:
            if is_owned: st.caption(f"💼 **보유** | 평단:{avg_price:,.0f} | 수량:{quantity}")
            else: st.caption(f"👀 **관심**")
        with col_btn:
