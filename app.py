        ...
        if not rec_news:
            ...
            if len(passed_results) == 0:
                ...
                with st.spinner(f"[4단계] 최종 선별된 {len(passed_results)}개 중 시니어 애널리스트 방식 Top 3 보고서 작성 중..."):
                    ...
                    # app (20).py 내 탭 4의 step3_prompt 정의 부분을 아래 코드로 교체하세요.
step3_prompt = (
    f"당신은 정교한 퀀트 가치평가 시스템을 운영하는 리서치 센터장입니다.\n"
    f"제공된 파이썬 실행 로그(재무 데이터, 밸류에이션 체제, 신뢰도 등)를 바탕으로 신뢰성 있는 추천 리포트를 작성하세요.\n\n"
    f"=== 리포트 작성 포맷 ===\n"
    f"### 📌 [종목명] ([티커])\n"  # <- 이 라인이 AI 본문에 종목 코드를 강제 출력하게 만들어 파서와 스크랩 버튼을 살립니다.
    f"**🎯 투자의견: [매수 / 유지 / 비중축소 / 손절]**\n"
    f"- **현재가:** 000원\n"
    f"- **단기 목표가(기술적 상단):** 00원\n"
    f"- **중기 적정가(성장 가치):** 00원\n"
    f"- **시스템 손절가:** [투자 기간] 손절가 00원\n"  # <- 에러를 유발하던 중괄호({})를 대괄호([]) 문구로 안전하게 수정했습니다.
    f"- **💡 3x3 매트릭스 진단:** [Tier 등급 기입] (로그를 참고하여 이 등급이 부여된 이유를 '목표가 변동 + 실적' 형태로 1줄 설명)\n"
    ...
    f"========================\n\n"
    f"[SELECTED_TICKERS]: 종목들의 6자리 코드를 쉼표로 구분하여 여기에 다시 적어주세요.\n"
)
