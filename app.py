            ...
            if len(valid_results) == 0:
                ...
                with st.spinner(f"[4단계] 최종 선별된 {len(valid_results)}개 중 시니어 애널리스트 방식 Top 3 보고서 작성 중..."):
                    ...
                    step3_prompt = (
                        ...
                        f"3. [숫자 일치 강제 - 매우 중요] 리포트에 기재하는 모든 가격(현재가, 목표가, 손절가 등)과 실적 수치는 **'파이썬 퀀트 엔진 연산 로그'에 찍힌 숫자를 절대 반올림하거나 가공하지 말고 1원 단위까지 똑같이 복사해서 기재**하십시오.\n"
                        f"4. [스토리텔링] 파편화된 데이터(뉴스, 수급, 밸류에이션, 실적 추이)를 단순 나열하지 말고, **하나의 투자 논리(Investment Story)**로 연결하여 서술하십시오.\n"
                        f"5. [추세 변화] 실적(매출/영업이익)의 분기별 증감 추세와 외국인/기관의 '최근 5일 vs 20일 수급 변화'를 바탕으로 모멘텀을 분석하십시오.\n"
                        f"6. [자기 검증] 스스로 도출한 결론이 틀릴 가능성(반대 논리 및 Value Trap 리스크) 3가지를 반드시 서술하십시오.\n"
                        f"7. [액션 플랜] 마지막에 투자 의견, 매수가/목표가/손절가, 향후 확인해야 할 이벤트를 반드시 요약하십시오.\n\n"
                        f"=== 리포트 작성 포맷 ===\n"
